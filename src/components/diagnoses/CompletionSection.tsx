...
export const CompletionSection = memo(function CompletionSection({
  ...
  if (step === "user") {
    if (isComplete) {
      return (
        <div className={`rounded-[40px] border border-white/70 bg-white/90 backdrop-blur-2xl shadow-[0px_24px_64px_rgba(0,0,0,0.12),0px_12px_32px_rgba(0,0,0,0.08),inset_0px_1px_0px_rgba(255,255,255,0.8)] ${isMobile ? "space-y-4 p-8" : "space-y-5 p-10"}`}>
          <p className={`font-['Coming_Soon:Regular',sans-serif] font-normal text-black text-shadow-[0px_2px_4px_rgba(0,0,0,0.1)] ${isMobile ? "text-base" : "text-lg"}`}>
            自分の回答がそろいました！<br />
    ...
    return (
      <p className={`font-['Coming_Soon:Regular',sans-serif] font-normal text-black/60 text-shadow-[0px_2px_4px_rgba(0,0,0,0.1)] ${isMobile ? "text-xs" : "text-sm"}`}>
